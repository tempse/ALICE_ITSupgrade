...
{   
  ...
  if(gROOT->GetVersion() == problematicRootVersion) {
   ...
   Int_t app_IsTaggedRPConv_classicalCuts_prefilter;
   ...
   theTree->SetBranchAddress( "IsTaggedRPConv_classicalCuts_prefilter", &app_IsTaggedRPConv_classicalCuts_prefilter );
   ...
   for (Long64_t ievt=0; ievt<nIevt;ievt++) {
      ...
      if(app_IsTaggedRPConv_classicalCuts_prefilter==1) {
	BDT = 999;
      }else if(app_mass < .1) {
	BDT = 99;
      }else {
