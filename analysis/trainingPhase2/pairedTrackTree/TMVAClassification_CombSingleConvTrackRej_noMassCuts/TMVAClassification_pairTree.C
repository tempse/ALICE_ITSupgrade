...
int TMVAClassification_pairTree( TString myMethodList = "" )
{
   ...
   TFile *infile = TFile::Open("../FT2_AnalysisResults_Upgrade_addFeat_pairtree_us_part2_1-1-8-split.root");
   ...
   dataloader->AddVariable( "var_phiv := phiv-1.57", 'F' );
   dataloader->AddVariable( "var_mass := mass", 'F' );
   dataloader->AddVariable( "var_pz_diff := pz1/sqrt(px1*px1+py1*py1)-pz2/sqrt(px2*px2+py2*py2)", 'F' );
   dataloader->AddVariable( "var_sumz := sumz", 'F' );
   dataloader->AddVariable( "var_diffz := diffz-1.57", 'F' );
   dataloader->AddVariable( "var_opang := opang", 'F' );
   dataloader->AddVariable( "var_nITS1 := nITS1" );
   dataloader->AddVariable( "var_nITS2 := nITS2" );
   dataloader->AddVariable( "var_nITSshared1 := nITSshared1" );
   dataloader->AddVariable( "var_nITSshared2 := nITSshared2" );
   dataloader->AddVariable( "var_nTPC1 := nTPC1" );
   dataloader->AddVariable( "var_nTPC2 := nTPC2" );
   ...
   dataloader->AddVariable( "var_ITSchi21 := ITSchi21" );
   dataloader->AddVariable( "var_ITSchi22 := ITSchi22" );
   dataloader->AddVariable( "var_TPCchi21 := TPCchi21" );
   dataloader->AddVariable( "var_TPCchi22 := TPCchi22" );
   ...
   dataloader->AddSpectator( "IsTaggedRPConv_classicalCuts" );
   dataloader->AddSpectator( "IsTaggedRPConv_classicalCuts_prefilter" );
   dataloader->AddSpectator( "IsRPConv_MVAoutput" );
   dataloader->AddSpectator( "IsTaggedRPConv_MVAcuts_prefilter" );
   dataloader->AddSpectator( "IsTaggedConvTrack1" );
   dataloader->AddSpectator( "IsTaggedConvTrack2" );
...
TCut signalCut = "!(IsRP==0 && (IsTaggedConvTrack1==1 || IsTaggedConvTrack2==1))"; // how to identify signal events
TCut backgrCut = "IsRP==0 && (IsTaggedConvTrack1==1 || IsTaggedConvTrack2==1)"; // how to identify background events  
...
//       factory->PrepareTrainingAndTestTree( mycut,       "NTrain_Signal=3751:NTrain_Background=52925:NTest_Signal=0:NTest_Background=0:SplitMode=Random:!V" );
   dataloader->PrepareTrainingAndTestTree( mycut, "!V:SplitMode=random:NTest_Signal=5000:NTest_Background=5000" );
   ...
   if (Use["MLP"])
      factory->BookMethod( dataloader, TMVA::Types::kMLP, "MLP", "!H:!V:NeuronType=tanh:VarTransform=N:ConvergenceTests=50:NCycles=500:HiddenLayers=N,N,N:UseRegulator" );
   ...
   if (Use["BDTG"]) // Gradient Boost
    ...
    if (Use["BDT"])  // Adaptive Boost
      factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDT",
                           "!H:!V:NTrees=800:DoBoostMonitor=True:MinNodeSize=5%:MaxDepth=3:BoostType=AdaBoost:AdaBoostBeta=0.05:UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=GiniIndex:nCuts=20:VarTransform=N" );
