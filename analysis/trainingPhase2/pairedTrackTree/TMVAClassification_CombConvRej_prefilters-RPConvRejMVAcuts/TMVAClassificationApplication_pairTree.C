...
{   
  ...
  if(gROOT->GetVersion() == problematicRootVersion) {
   ...
   Int_t app_IsTaggedRPConv_MVAcuts_prefilter;
   ...
   theTree->SetBranchAddress( "IsTaggedRPConv_MVAcuts_prefilter", &app_IsTaggedRPConv_MVAcuts_prefilter );
   ...
   for (Long64_t ievt=0; ievt<nIevt;ievt++) {
      ...
      if(app_IsTaggedRPConv_MVAcuts_prefilter==1) {
	BDT = 999;
      }else if(app_mass < .1) {
	BDT = 99;
      }else {
