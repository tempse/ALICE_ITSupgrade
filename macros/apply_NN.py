...
model_filename = 'temp_output/ann/keras_weights/weights_final.hdf5'
...
scaler_attributes_filename = 'temp_output/ann/StandardScaler_attributes.pkl'

predictions_filename = 'temp_output/ann/predictions_NN.root'
...
Yscore = clf.predict_proba(Xapp.as_matrix())

"""
n_chunks = 20
Yscore = np.empty((0,1), dtype=np.float32)
for i in range(0, n_chunks if (Xapp.shape[0]%n_chunks==0) else n_chunks+1):
    ...
    print(Yscore.shape, Xapp.iloc[start:stop,:].shape, clf.predict_proba(Xapp.iloc[start:stop,:].as_matrix()).shape)
