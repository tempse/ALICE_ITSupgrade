...
{   
  ...
  if(gROOT->GetVersion() == problematicRootVersion) {
   ...
   reader->AddVariable( "var_nITS := abs(nITS)", &var_nITS );
   reader->AddVariable( "var_nTPC := abs(nTPC)", &var_nTPC );
   ...
   //   
   TFile *input(0);
   TString fname = "../../inputData/FT2_AnalysisResults_Upgrade_addFeat_test_1-10-split.root";
   ...
   for (Long64_t ievt=0; ievt<nIevt;ievt++) {
      ...
      var_nITS = TMath::Abs(app_nITS);
      var_nTPC = TMath::Abs(app_nTPC);
      var_nITSshared = TMath::Abs(app_nITSshared);
      var_ITSchi2 = TMath::Abs(app_ITSchi2);
      var_TPCchi2 = TMath::Abs(app_TPCchi2);
